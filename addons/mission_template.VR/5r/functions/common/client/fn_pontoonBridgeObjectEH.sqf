...
["acex_fortify_objectPlaced", {
	...
	if (((typeOf _obj) in _thisArgs) && (_player == player)) then {
        ...
        private _matches = _nearby select { _x getVariable ["x5r_bridgeObj", false] };
		if ((count _matches) > 0) then {
			// Consume requisite item if found and create subpiece array variable
			[_obj,_obj] call x5r_Common_fnc_pontoonBridgeAttachActions;
			deleteVehicle _matches select 0;
			_obj enableSimulationGlobal false;
			_obj allowDamage false;
			[_obj, true] call zen_building_markers_fnc_set;
			private _x5r_bridgeChildren = [];
			_obj setVariable ["x5r_bridgeChildren",_x5r_bridgeChildren,true];
		} else {
...
["acex_fortify_objectDeleted", {
	...
	if (((typeOf _obj) in _thisArgs) && (_player == player)) then {
		...
		[_crate] call x5r_Common_fnc_pontoonBridgeCreateObject; 
		{deleteVehicle _x;} forEach (_obj getVariable "x5r_bridgeChildren");
	}
},_pontoonObject] call CBA_fnc_addEventHandlerArgs;