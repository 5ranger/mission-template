5r_arsenalItems = [
    "tfw_ilbe_whip_coy",
    "rhsusf_ANPVS_15",
    "rhsusf_cvc_green_krh_ess",
    "rhsusf_bino_lerca_1200_black",
    "Tier1_Shortdot_ADM_Black",
    "rhsusf_acc_anpeq15_top",
    "ACE_muzzle_mzls_smg_01",
    "rhsusf_acc_tdstubby_blk",
    "rhs_mag_30Rnd_556x45_M855A1_Stanag",
    "5R_Tank_Helmet_27_ESS_WINT",
    "rhsusf_hgu56p",
    "5R_Tank_Helmet_25_ESS_ARD",
    "ACE_Chemlight_HiGreen",
    "UK3CB_G_Tactical_Gloves_Green",
    "Tier1_Shortdot_Geissele_Docter_Tan",
    "Tier1_Romeo4T_BCQ_G33_Riser_Tan_Up",
    "Tier1_Elcan_156_C1_ARD_Docter_Black_2D",
    "Tier1_EXPS3_0_G33_Black_Up",
    "rhsusf_acc_T1_high",
    "Tier1_URX4_NGAL_M600V_alt",
    "5R_Tank_Helmet_21_ESS_ARD_Clean",
    "VirtusHelmet_10",
    "mpx_virtus_taco2",
    "5R_Tank_Helmet_27",
    "ACE_HandFlare_White",
    "rhsusf_acc_su230_c",
    "UK3CB_G_Bandanna_green_check",
    "VirtusHelmet_3",
    "Tier1_Eotech551_3xMag_Riser_Black_Up",
    "Tier1_EXPS3_0_3xMag_Tano_Up",
    "Tier1_MicroT1_Low_Black",
    "Tier1_MicroT2_Leap_Riser_Tan",
    "Tier1_MicroT2_3xMag_Tan_Up",
    "Tier1_Mk18_LA5_M603V_FL",
    "UK3CB_BAF_LLM_IR_Black",
    "5R_Tank_Helmet_24_Alt",
    "UK3CB_G_Bandanna_aviator_winter",
    "UK3CB_BAF_HMNVS",
    "ACE_bodyBag_blue",
    "5R_Tank_Helmet_25_Alt_WINT",
    "ACE_quikclot",
    "ACE_Canteen",
    "ItemcTabHCam",
    "Tier1_MicroT2_Leap_3xMag_Riser_Tan_Up",
    "Tier1_Eotech551_Riser_Black",
    "Tier1_URX4_NGAL_Top",
    "Tier1_URX4_LA5_M300C",
    "UK3CB_BAF_H_Beret_AAC_Over",
    "B_Kitbag_mcamo",
    "Tier1_SIG_MCX_115_Virtus",
    "5R_Tank_Helmet_24_WINT",
    "kat_IO_FAST",
    "Tier1_LeupoldM3A_Geissele_Desert",
    "Tier1_EXPS3_0_G33_Riser_Desert_Up",
    "Tier1_SIG_MCX_115_Virtus_Black",
    "ACE_HandFlare_Green",
    "ItemAndroidMisc",
    "ACE_HuntIR_M203",
    "UK3CB_G_Ballistic_Black_Gloves_Green_Headset",
    "UK3CB_BAF_SpecterLDS",
    "Tier1_Razor_Gen3_110_Geissele_Docter",
    "SmokeShellOrange",
    "mpx_daysack3",
    "kat_lidocaine",
    "5R_Tank_Helmet_25_WINT",
    "VirtusHelmet_9",
    "rhsusf_hgu56p_mask",
    "5R_Tank_Helmet_21_ARD",
    "rhsusf_cvc_winter_krh_ess",
    "Tier1_Romeo4T_BCQ_Black",
    "Tier1_URX4_NGAL_M300C",
    "UK3CB_underbarrel_acc_afg_t",
    "5R_Tank_Helmet_25_ESS_WINT",
    "ACE_40mm_Flare_red",
    "ACE_Chemlight_Shield",
    "Tier1_SIG_MCX_115_Virtus_Desert",
    "tfw_ilbe_DD_coy",
    "rhsusf_ihadss",
    "kat_larynx",
    "UK3CB_G_Bandanna_winter",
    "UK3CB_G_Tactical_Black",
    "ACE_Chemlight_HiRed",
    "ACE_MapTools",
    "5R_Tank_Helmet_24_Alt_ARD",
    "Tier1_Romeo4T_BCD_G33_Riser_Tan_Up",
    "rhsusf_acc_g33_T1",
    "Tier1_URX4_LA5_M603V",
    "5R_Tank_Helmet_22_Alt",
    "ItemWatch",
    "rhsusf_oakley_goggles_clr",
    "UK3CB_G_Balaclava2_BLK",
    "5R_Tank_Helmet_22_ARD",
    "Tier1_MicroT2_Low_Tan",
    "Tier1_Elcan_156_C2_ARD_Docter_FDE_2D",
    "Tier1_Elcan_156_C1_Docter_Black_2D",
    "rhsusf_acc_anpeq15side",
    "rhsusf_acc_anpeq15_bk_top",
    "UK3CB_G_Tactical_Clear_Tactical_Gloves_Tan",
    "UK3CB_G_Tactical_Black_Tactical_Gloves_Tan",
    "5R_Tank_Helmet_27_ESS_ARD",
    "kat_Caffeine",
    "mpx_virtus3",
    "5R_Tank_Helmet_24_ESS",
    "mpx_daysack5",
    "Tier1_MicroT2_3xMag_Riser_Tan_Up",
    "Tier1_MicroT2_Leap_G33_Riser_Black_Up",
    "Tier1_Elcan_156_C1_ARD_Docter_FDE",
    "Tier1_KAC_556_QDC_Tan",
    "G_Lowprofile",
    "UK3CB_BAF_L16",
    "ItemcTabMisc",
    "kat_Armband_Kat_Item",
    "tfw_ilbe_whip_gr",
    "UK3CB_G_Ballistic_Black_Tactical_Gloves_Black_Headset",
    "5R_Tank_Helmet_27_ESS",
    "Tier1_Eotech553_3xMag_Black_Up",
    "Tier1_EXPS3_0_3xMag_Riser_Tano_Up",
    "rhsusf_acc_ACOG_wd",
    "TFAR_anprc152",
    "ItemGPS",
    "kat_bloodIV_O_N",
    "B_Parachute",
    "5R_Tank_Helmet_26_ESS_ARD_Clean",
    "rhsusf_oakley_goggles_ylw",
    "Tier1_Romeo4T_BCD_Low_Tan",
    "mpx_virtus_admin",
    "UK3CB_G_Ballistic_Black_Gloves_Black_Headset",
    "ACE_M84",
    "5R_Tank_Helmet_21_Alt",
    "Tier1_MicroT2_G33_Riser_Desert_Up",
    "Tier1_Razor_Gen2_16_ADM",
    "Tier1_Mk18_LA5_M603V",
    "Tier1_Mk18_LA5_M600V_Black",
    "Tier1_URX4_NGAL_M600V_alt_Black",
    "UK3CB_underbarrel_acc_grippod_t",
    "rhs_mag_m715_Green",
    "UK3CB_G_Bandanna_flora_alt",
    "UK3CB_G_Balaclava2_DES",
    "UK3CB_BAF_MaxiKite",
    "RKSL_optic_LDS_C",
    "5R_Tank_Helmet_22_ESS_ARD",
    "ACE_Chemlight_HiWhite",
    "5R_Tank_Helmet_26",
    "UK3CB_G_Tactical_Clear",
    "VirtusHelmet_2",
    "Tier1_Romeo4T_BCQ_Tan",
    "Tier1_ATACR18_Geissele_Black",
    "Tier1_Razor_Gen2_16",
    "UK3CB_BAF_LLM_Flashlight_Black",
    "Tier1_URX4_NGAL_M600V_Black_FL",
    "UK3CB_BAF_762_200Rnd_T",
    "rhsusf_hgu56p_visor_mask",
    "tfw_ilbe_DD_mc",
    "Tier1_ATACR18_Geissele_Docter_Black",
    "Laserdesignator_03",
    "Chemlight_green",
    "Tier1_EXPS3_0_3xMag_Riser_Desert_Up",
    "Tier1_ATACR18_Geissele_Desert",
    "Tier1_SIG_MCX_115_Virtus_FDE",
    "UK3CB_BAF_762_200Rnd",
    "mpx_virtus_ato_p",
    "rhs_ec75_mag",
    "Virtus_D_4",
    "UK3CB_BAF_Javelin_CLU",
    "5R_Beret_KRH",
    "Tier1_Eotech551_Black",
    "Tier1_Shortdot_Geissele_Tan",
    "UK3CB_BAF_TA31F",
    "Tier1_URX4_NGAL_M600V_Black",
    "ACE_muzzle_mzls_93mmg",
    "UK3CB_BAF_Silencer_L85",
    "Tier1_BCM_Gunfighter_VG_Black",
    "Tier1_Eotech551_3xMag_Black_Up",
    "rhs_mag_m713_Red",
    "rhsusf_bino_lerca_1200_tan",
    "VirtusHelmet_8",
    "UK3CB_BAF_L110_762",
    "kat_amiodarone",
    "mpx_virtus_taco2_p",
    "Tier1_Eotech553_Black",
    "Tier1_LeupoldM3A_ADM_Tan",
    "rhsusf_acc_compm4",
    "Tier1_MicroT2_Leap_G33_Riser_TanBlack_Up",
    "Tier1_URX4_NGAL_M600V_alt_FL",
    "UK3CB_BAF_L85A3",
    "UK3CB_BAF_AT4_CS_AP_Launcher",
    "rhsusf_shemagh2_gogg_tan",
    "rhs_weap_mk18_m320",
    "UK3CB_G_Tactical_Black_Gloves_Black",
    "UK3CB_G_Tactical_Gloves_Black",
    "UK3CB_BAF_L131A1",
    "B_UavTerminal",
    "Tier1_MicroT2_Leap_3xMag_Tan_Up",
    "UK3CB_G_Gloves_Black",
    "UK3CB_G_Tactical_Black_Tactical_Gloves_Black",
    "Tier1_Eotech551_Riser_Desert",
    "Tier1_Eotech551_3xMag_Desert_Up",
    "rhsusf_acc_grip3",
    "ACE_EntrenchingTool",
    "rhsusf_hgu56p_visor_mask_black",
    "UK3CB_BAF_L85A3_UGL",
    "Tier1_Razor_Gen3_110_ADM_T2",
    "5R_Tank_Helmet_23_ARD",
    "5R_Tank_Helmet_25_ESS",
    "rhs_weap_mk18_KAC",
    "UK3CB_G_Bandanna_aviator_brown_check",
    "tfw_ilbe_blade_black",
    "Tier1_MicroT2_Leap_G33_Desert_Up",
    "Tier1_Shortdot_Geissele_Docter_Desert",
    "Tier1_Romeo4T_BCD_G33_Riser_TanBlack_Up",
    "rhsusf_acc_anpeq15side_bk",
    "Tier1_URX4_LA5_Top",
    "Tier1_KAC_VFG_MLOK_Black",
    "mpx_virtus2",
    "ACE_artilleryTable",
    "SmokeShellYellow",
    "Tier1_Eotech553_L3_Black_Up",
    "Tier1_URX4_NGAL_Side",
    "5R_Tank_Helmet_22_ESS_ARD_Clean",
    "5R_Tank_Helmet_23_Alt",
    "rhs_googles_yellow",
    "rhs_mag_m714_White",
    "UK3CB_BAF_9_17Rnd",
    "Tier1_MicroT2_Riser_Black",
    "Tier1_MicroT2_Leap_G33_Tan_Up",
    "UK3CB_BAF_SUSAT",
    "Tier1_EXPS3_0_3xMag_Black_Up",
    "Tier1_MVG_MLOK_Black",
    "5R_Tank_Helmet_23_ESS_ARD",
    "Tier1_MicroT1_Leap_3xMag_Desert_Up",
    "Virtus_B_3",
    "rhsusf_oakley_goggles_blk",
    "5R_Tank_Helmet_21_Alt_ARD",
    "Tier1_Romeo4T_BCQ_Low_Black",
    "Tier1_MicroT2_Leap_Riser_Black",
    "rhsusf_acc_su230_mrds",
    "Tier1_MRDS_Tan",
    "Tier1_GripPod_MLOK_Black",
    "UK3CB_underbarrel_acc_afg",
    "ACE_SpraypaintRed",
    "kat_vacuum",
    "kat_EACA",
    "5R_Tank_Helmet_23_Alt_WINT",
    "mpx_daysack8",
    "Chemlight_red",
    "5R_Tank_Helmet_24_ESS_ARD_Clean",
    "UK3CB_G_Tactical_Clear_Tactical_Gloves_Black",
    "Tier1_Elcan_156_C1_ARD_FDE",
    "Tier1_MicroT1_Leap_Riser_Black",
    "Tier1_URX4_NGAL_M600V_alt_Black_FL",
    "Tier1_URX4_LA5_M300C_FL",
    "ACE_muzzle_mzls_H",
    "rhsusf_hgu56p_mask_black",
    "mpx_virtus",
    "5R_Tank_Helmet_21",
    "5R_Tank_Helmet_27_Alt_WINT",
    "kat_atropine",
    "rhsusf_m112_mag",
    "Tier1_SR16_Carbine_Mod2_LMT_Black",
    "VirtusHelmet_1",
    "Tier1_Eotech551_L3_Desert_Up",
    "Tier1_EXPS3_0_Black",
    "Tier1_Mk18_LA5_M600V_Black_FL",
    "kat_ncdKit",
    "ACE_bodyBag",
    "Tier1_LeupoldM3A_Geissele_Tan",
    "Tier1_URX4_NGAL_M603V",
    "5R_Tank_Helmet_26_ESS_ARD",
    "Tier1_Romeo4T_BCD_Low_Black",
    "5R_Tank_Helmet_25_Alt",
    "UK3CB_BAF_G_Tactical_Orange",
    "ACE_40mm_Flare_ir",
    "Tier1_MicroT1_Leap_Riser_Desert",
    "Tier1_Elcan_156_C1_ARD_Black_2D",
    "Tier1_Elcan_156_C2_FDE",
    "rhsusf_acc_g33_xps3",
    "Tier1_Elcan_156_C1_Docter_FDE",
    "ACE_muzzle_mzls_B",
    "rhsusf_acc_tdstubby_tan",
    "UK3CB_G_Ballistic_Black_Gloves_Green",
    "Tier1_Romeo4T_BCD_Riser_Black",
    "VirtusHelmet_7",
    "UK3CB_G_Tactical_Clear_Tactical_Gloves_Green",
    "UK3CB_G_Bandanna_aviator_winter_flora_alt",
    "rhs_googles_black",
    "Tier1_Romeo4T_BCQ_G33_Riser_Black_Up",
    "Tier1_Elcan_156_C2_Docter_FDE_2D",
    "Tier1_MicroT2_Leap_Tan",
    "Tier1_MicroT2_Leap_G33_Riser_Desert_Up",
    "Tier1_Eotech553_Tan",
    "UK3CB_BAF_LLM_IR_Tan",
    "Tier1_URX4_LA5_M300C_Black_FL",
    "5R_Tank_Helmet_23_ESS_WINT",
    "5R_Tank_Helmet_27_WINT",
    "kat_norepinephrine",
    "kat_TXA",
    "UK3CB_G_Tactical_Black_Gloves_Green",
    "ACE_plasmaIV",
    "rhsusf_hgu56p_visor_black",
    "ACE_adenosine",
    "ACE_DefusalKit",
    "5R_Tank_Helmet_22_ESS",
    "kat_fentanyl",
    "UK3CB_G_Ballistic_Black_Gloves_Black",
    "Laserdesignator",
    "Virtus_C_4",
    "Tier1_Elcan_156_C2_ARD_Black_2D",
    "Tier1_EXPS3_0_G33_Riser_Tano_Up",
    "rhsusf_acc_su230_mrds_c",
    "Tier1_Romeo4T_BCQ_G33_TanBlack_Up",
    "Tier1_URX4_NGAL_M300C_Black_FL",
    "Tier1_URX4_LA5_M600V_FL",
    "Tier1_Romeo4T_BCD_Tan",
    "Virtus_D_1",
    "kat_Pulseoximeter",
    "Virtus_B_4",
    "5R_Tank_Helmet_25_Alt_ARD",
    "ACE_salineIV_500",
    "ACE_plasmaIV_250",
    "rhsusf_hgu56p_visor_mask_green",
    "rhsusf_m112x4_mag",
    "kat_chestSeal",
    "UK3CB_BAF_L119A1",
    "UK3CB_BAF_U_CombatUniform_MTP",
    "UK3CB_BAF_L7A2",
    "UK3CB_G_Gloves_Tan",
    "Tier1_Romeo4T_BCQ_G33_Tan_Up",
    "mpx_daysack7",
    "rhs_mag_m716_yellow",
    "mpx_camel",
    "Tier1_LeupoldM3A_ADM_T2_Black",
    "mpx_virtus_Medic",
    "UK3CB_BAF_L119A1_CQB",
    "Tier1_MicroT1_Leap_Desert",
    "Tier1_SIG_Romeo1_RIS",
    "Tier1_Elcan_156_C2_ARD_Docter_Black_2D",
    "Tier1_Gangster_Grip_Black",
    "Tier1_Harris_Bipod_MVG_MLOK_Black",
    "5R_Tank_Helmet_21_Alt_WINT",
    "ACE_Chemlight_White",
    "rhs_googles_orange",
    "Virtus_C_2",
    "rhsusf_hgu56p_black",
    "G_Aviator",
    "ACE_packingBandage",
    "Virtus_A_1",
    "5R_Tank_Helmet_23_ESS_ARD_Clean",
    "UK3CB_BAF_Tripod",
    "ACE_surgicalKit",
    "ACE_epinephrine",
    "Tier1_EXPS3_0_Riser_Black",
    "Tier1_Elcan_156_C1_Docter_Black",
    "Tier1_Razor_Gen2_16_Geissele",
    "Tier1_Eotech551_L3_Black_Up",
    "Tier1_Elcan_156_C2_Docter_Black",
    "Tier1_MVG_MLOK_FDE",
    "kat_bloodIV_O_N_500",
    "kat_X_AED",
    "rhsusf_hgu56p_visor",
    "Tier1_MRDS",
    "Virtus_B_2",
    "Tier1_MicroT2_3xMag_Riser_Black_Up",
    "Tier1_Shortdot_ADM_Desert",
    "ACE_muzzle_mzls_smg_02",
    "ACE_rope6",
    "ACE_40mm_Flare_white",
    "Chemlight_yellow",
    "UK3CB_BAF_H_Beret_AAC_PRR",
    "Tier1_Shortdot_Geissele_Desert",
    "mpx_daysack9",
    "rhsusf_acc_T1_low_fwd",
    "5R_Tank_Helmet_21_ESS",
    "Tier1_ATACR18_ADM_Black",
    "Tier1_Elcan_156_C2_Docter_FDE",
    "Tier1_EXPS3_0_G33_Riser_Black_Up",
    "Tier1_KAC_762_QDC_Black",
    "ACE_Chemlight_UltraHiOrange",
    "rhsusf_hgu56p_green",
    "Tier1_Eotech551_Desert",
    "rhsusf_cvc_green_alt_krh_helmet",
    "rhs_weap_mk18_KAC_bk",
    "Tier1_MicroT2_G33_Riser_Tan_Up",
    "Tier1_MicroT2_Leap_G33_TanBlack_Up",
    "Tier1_ATACR18_ADM_T1_Black",
    "Tier1_URX4_NGAL_M600V_FL",
    "G_Bandanna_oli",
    "Tier1_Elcan_156_C1_FDE",
    "UK3CB_G_Tactical_Clear_Gloves_Tan",
    "kat_pocketBVM",
    "UK3CB_BAF_L22A2",
    "rhssaf_veil_Green",
    "ACE_EarPlugs",
    "G_Bandanna_aviator",
    "5R_Tank_Helmet_24_Alt_WINT",
    "5R_Tank_Helmet_22_ESS_WINT",
    "Tier1_LeupoldM3A_ADM_T2_Desert",
    "rhsusf_acc_ACOG_d",
    "Tier1_Shortdot_Geissele_Docter_Black",
    "Tier1_URX4_LA5_M600V",
    "Tier1_KAC_VFG_MLOK_DE",
    "UK3CB_BAF_H_Beret_RTR_PRR_Over",
    "UK3CB_G_Bandanna_winter_flora_alt",
    "UK3CB_G_Tactical_Black_Gloves_Tan",
    "KAT_Empty_bloodIV_250",
    "UK3CB_G_Ballistic_Black_Tactical_Gloves_Green_Headset",
    "Tier1_MicroT2_Leap_3xMag_Black_Up",
    "Tier1_Romeo4T_BCQ_Low_Tan",
    "rhsusf_acc_g33_xps3_tan",
    "rhsusf_acc_T1_low",
    "mpx_virtus_taco_p",
    "tfw_ilbe_blade_gr",
    "UK3CB_G_Tactical_Clear_Gloves_Black",
    "ACE_Flashlight_XL50",
    "rhs_weap_mk18_bk",
    "RKSL_optic_LDS",
    "rhs_mag_m67",
    "UK3CB_BAF_G_Tactical_Clear",
    "G_Bandanna_khk",
    "rhsusf_shemagh_gogg_tan",
    "VirtusHelmet_6",
    "rhsusf_hgu56p_mask_green",
    "Tier1_MicroT2_Leap_G33_Black_Up",
    "Tier1_Eotech553_L3_TanBlack_Up",
    "UK3CB_BAF_LLM_Flashlight_Tan",
    "Tier1_KAC_762_QDC_Tan",
    "Tier1_BCM_Gunfighter_VG_FDE",
    "G_Combat_Goggles_tna_F",
    "kat_IV_16",
    "tfw_ilbe_blade_coy",
    "rhs_mag_M433_HEDP",
    "ItemMicroDAGRMisc",
    "ACE_NonSteerableParachute",
    "ACE_HuntIR_monitor",
    "UK3CB_BAF_762_L42A1_20Rnd",
    "ACE_microDAGR",
    "Tier1_Elcan_156_C1_ARD_Docter_FDE_2D",
    "Tier1_URX4_NGAL_M300C_Black",
    "5R_Tank_Helmet_22_Alt_WINT",
    "rhs_mag_m661_green",
    "Tier1_MicroT1_Low_Desert",
    "ACE_rope12",
    "UK3CB_BAF_U_CrewmanCoveralls_RTR",
    "5R_Beret_RANGER",
    "UK3CB_G_Ballistic_Black_Tactical_Gloves_Black",
    "UK3CB_G_Ballistic_Black_Tactical_Gloves_Tan_Headset",
    "5R_Tank_Helmet_21_WINT",
    "ACE_SpraypaintGreen",
    "UK3CB_G_Ballistic_Black_Gloves_Tan_Headset",
    "rhsusf_acc_anpeq16a_top",
    "Tier1_KAC_556_QDC_Black",
    "Tier1_Gangster_Grip_Tan",
    "5R_Tank_Helmet_27_Alt_ARD",
    "rhs_googles_clear",
    "kat_Penthrox",
    "ACE_40mm_Flare_green",
    "5R_Tank_Helmet_26_Alt",
    "kat_aatKit",
    "Tier1_EXPS3_0_Riser_Tano",
    "Tier1_MicroT2_Leap_Black",
    "Tier1_URX4_LA5_M600V_alt_Black",
    "Virtus_C_3",
    "UK3CB_G_Ballistic_Black_Tactical_Gloves_Green",
    "Virtus_A_2",
    "ACE_personalAidKit",
    "Tier1_Elcan_156_C2_ARD_Docter_FDE",
    "Tier1_SIG_Romeo1",
    "Tier1_Eotech553_3xMag_Tan_Up",
    "Tier1_URX4_LA5_M600V_Black_FL",
    "ACE_muzzle_mzls_338",
    "UK3CB_underbarrel_acc_afg_d",
    "UK3CB_G_Ballistic_Black_Gloves_Tan",
    "rhs_mag_30Rnd_556x45_M855A1_Stanag_Tracer_Red",
    "KAT_Empty_bloodIV_500",
    "UK3CB_BAF_L119A1_RIS",
    "rhsusf_cvc_green_krh_helmet",
    "5R_Tank_Helmet_24_ESS_ARD",
    "Virtus_B_1",
    "Tier1_Romeo4T_BCD_G33_Tan_Up",
    "Tier1_Romeo4T_BCQ_G33_Riser_TanBlack_Up",
    "Tier1_Elcan_156_C2_ARD_Docter_Black",
    "Tier1_EXPS3_0_Riser_Desert",
    "Tier1_URX4_LA5_Side",
    "Tier1_URX4_LA5_M600V_alt",
    "mpx_daysack",
    "5R_Beret_KRH_PRR",
    "tfw_ilbe_whip_mc",
    "UK3CB_BAF_762_100Rnd",
    "ACE_VMM3",
    "5R_Tank_Helmet_26_ESS",
    "UK3CB_BAF_G_Tactical_Grey",
    "UK3CB_G_Tactical_Black_Tactical_Gloves_Green",
    "ACE_IR_Strobe_Item",
    "Tier1_MicroT2_G33_Black_Up",
    "Tier1_LeupoldM3A_ADM_Black",
    "Tier1_Elcan_156_C1_ARD_FDE_2D",
    "Tier1_EXPS3_0_3xMag_Desert_Up",
    "Tier1_KAC_556_QDC_CQB_Black",
    "UK3CB_BAF_SFFH",
    "UK3CB_BAF_762_L42A1_20Rnd_T",
    "VirtusHelmet_14",
    "5R_Tank_Helmet_26_WINT",
    "Virtus_A_4",
    "mpx_virtus_admin_p",
    "UK3CB_BAF_G_Tactical_Yellow",
    "5R_Tank_Helmet_23",
    "5R_Tank_Helmet_21_ESS_WINT",
    "tfw_ilbe_blade_mc",
    "SmokeShellGreen",
    "UK3CB_BAF_L111A1",
    "Tier1_Razor_Gen2_16_Geissele_Docter",
    "Tier1_ATACR18_ADM_Desert",
    "Tier1_Romeo4T_BCQ_Riser_Tan",
    "Tier1_KAC_556_QDC_CQB_Tan",
    "UK3CB_BAF_Silencer_L115A3",
    "5R_Tank_Helmet_21_ESS_ARD",
    "Tier1_SR16_Carbine_Mod2_LMT_FDE",
    "rhs_mag_M441_HE",
    "Tier1_ATACR18_ADM_T1_Desert",
    "UK3CB_G_Bandanna_aviator_flora_alt",
    "UK3CB_G_Bandanna_brown_check",
    "rhs_mag_M583A1_white",
    "UK3CB_BAF_U_CombatUniform_MTP_ShortSleeve",
    "rhsusf_acc_su230a_mrds",
    "Tier1_EXPS3_0_Tano",
    "UK3CB_underbarrel_acc_bipod",
    "mpx_virtus2_p",
    "ACE_salineIV_250",
    "5R_Beret_RANGER_PRR",
    "ToolKit",
    "Tier1_Razor_Gen3_110_Geissele",
    "UK3CB_underbarrel_acc_grippod_d",
    "5R_Tank_Helmet_27_ARD",
    "Laserdesignator_01_khk_F",
    "ItemMicroDAGR",
    "ACE_M14",
    "5R_Tank_Helmet_25",
    "UK3CB_G_Ballistic_Black",
    "Tier1_MicroT2_G33_Desert_Up",
    "mpx_virtus3_p",
    "ACE_morphine",
    "Tier1_SR16_CQB_Mod2_LMT_FDE",
    "UK3CB_BAF_H_Beret_RTR",
    "VirtusHelmet_5",
    "5R_Tank_Helmet_25_ARD",
    "rhsusf_cvc_krh_helmet",
    "VirtusHelmet_12",
    "Tier1_Razor_Gen3_110_ADM",
    "Tier1_URX4_NGAL_M600V",
    "tfw_ilbe_whip_black",
    "B_AssaultPack_mcamo",
    "Tier1_MicroT2_Riser_Tan",
    "mpx_virtus_taco",
    "Tier1_Elcan_156_C2_ARD_FDE",
    "Tier1_MicroT2_Tan",
    "Tier1_MicroT2_Leap_G33_Riser_Tan_Up",
    "Virtus_D_3",
    "Tier1_ATACR18_Geissele_Docter_Desert",
    "kat_epinephrineIV",
    "ACE_tourniquet",
    "Tier1_Elcan_156_C1_Docter_FDE_2D",
    "tfw_ilbe_DD_black",
    "Tier1_Romeo4T_BCD_G33_TanBlack_Up",
    "Tier1_Shortdot_Geissele_Black",
    "Tier1_MicroT2_Low_Black",
    "Tier1_Elcan_156_C1_ARD_Black",
    "uk3cb_acc_surefiregrip",
    "UK3CB_G_Gloves_Green",
    "mpx_boonie",
    "kat_guedel",
    "kat_bloodIV_O_N_250",
    "G_Squares_Tinted",
    "UK3CB_BAF_G_Tactical_Black",
    "kat_Bubble_Wrap",
    "Tier1_LeupoldM3A_Geissele_Docter_Desert",
    "Tier1_MicroT1_Leap_3xMag_Black_Up",
    "Tier1_Romeo4T_BCQ_G33_Black_Up",
    "Tier1_Romeo4T_BCQ_Riser_Black",
    "Tier1_GripPod_Tan",
    "ACE_Chemlight_IR",
    "Tier1_Elcan_156_C2_ARD_Black",
    "5R_Tank_Helmet_27_Alt",
    "UK3CB_G_Bandanna_white_check",
    "5R_Tank_Helmet_22_WINT",
    "ACE_UAVBattery",
    "5R_Tank_Helmet_22_Alt_ARD",
    "Tier1_MicroT2_3xMag_Black_Up",
    "rhs_weap_mk18",
    "rhsusf_cvc_winter_krh_helmet",
    "Virtus_A_3",
    "rhsusf_cvc_winter_alt_krh_helmet",
    "SmokeShellRed",
    "Tier1_LeupoldM3A_ADM_T2_Tan",
    "Tier1_LeupoldM3A_Geissele_Black",
    "Tier1_Elcan_156_C1_Black",
    "Tier1_EXPS3_0_Desert",
    "Tier1_Elcan_156_C2_ARD_FDE_2D",
    "rhsusf_acc_grip3_tan",
    "rhs_mag_m662_red",
    "ACE_wirecutter",
    "ItemMap",
    "B_UAV_01_backpack_F",
    "UK3CB_BAF_L129A1",
    "kat_accuvac",
    "Laserbatteries",
    "rhs_weap_mk18_KAC_d",
    "kat_suction",
    "Tier1_URX4_LA5_M600V_alt_Black_FL",
    "Tier1_URX4_LA5_M300C_Black",
    "G_Squares",
    "ACE_Altimeter",
    "rhsusf_hgu56p_visor_green",
    "5R_Beret_KRH_PRR_Over",
    "UK3CB_G_Ballistic_Black_Tactical_Gloves_Tan",
    "UK3CB_BAF_L16_Tripod",
    "UK3CB_BAF_762_100Rnd_T",
    "ACE_Chemlight_Orange",
    "Tier1_Elcan_156_C2_FDE_2D",
    "5R_Tank_Helmet_24_ESS_WINT",
    "ACE_fieldDressing",
    "ACE_Vector",
    "VirtusHelmet_15",
    "kat_oxygenTank_300",
    "5R_Tank_Helmet_22",
    "kat_BVM",
    "Tier1_MicroT1_Leap_3xMag_Riser_Desert_Up",
    "Tier1_KAC_762_DSR",
    "UK3CB_G_Tactical_Gloves_Tan",
    "rhs_mag_M397_HET",
    "rhs_ess_black",
    "kat_Painkiller",
    "5R_Tank_Helmet_26_Alt_WINT",
    "rhsusf_ANPVS_14",
    "ACE_Chemlight_HiYellow",
    "5R_Tank_Helmet_23_Alt_ARD",
    "UK3CB_G_Tactical_Clear_Gloves_Green",
    "Tier1_MicroT2_G33_Riser_Black_Up",
    "Tier1_Shortdot_ADM_Tan",
    "rhsusf_acc_ACOG_RMR",
    "rhs_weap_M320",
    "rhs_weap_mk18_d",
    "mpx_virtus_ato",
    "UK3CB_G_Face_Wrap_01",
    "rhsusf_cvc_krh_ess",
    "5R_Tank_Helmet_26_Alt_ARD",
    "G_Combat",
    "kat_nitroglycerin",
    "ACE_SpareBarrel",
    "kat_AED",
    "Tier1_Eotech551_3xMag_Riser_Desert_Up",
    "Tier1_Eotech551_L3_Riser_Desert_Up",
    "Tier1_LeupoldM3A_Geissele_Docter_Black",
    "VirtusHelmet_13",
    "kat_nalbuphine",
    "5R_Tank_Helmet_24_ARD",
    "Tier1_SR16_CQB_Mod2_LMT_Black",
    "5R_Beret_RANGER_PRR_Over",
    "ACE_elasticBandage",
    "5R_Tank_Helmet_24",
    "5R_Tank_Helmet_26_ARD",
    "G_Tactical_Clear",
    "UK3CB_G_Bandanna_aviator_green_check",
    "ACE_M26_Clacker",
    "Tier1_Elcan_156_C2_Black_2D",
    "VirtusHelmet_4",
    "ACE_plasmaIV_500",
    "Tier1_Elcan_156_C1_ARD_Docter_Black",
    "rhsusf_acc_anpeq16a",
    "Tier1_URX4_LA5_M600V_alt_FL",
    "kat_Carbonate",
    "ItemCompass",
    "5R_Tank_Helmet_25_ESS_ARD_Clean",
    "Tier1_LeupoldM3A_Geissele_Docter_Tan",
    "rhsusf_acc_ACOG_MDO",
    "Tier1_EXPS3_0_G33_Desert_Up",
    "rhsusf_acc_su230",
    "Tier1_URX4_NGAL_M300C_FL",
    "mpx_virtus4_p",
    "5R_Tank_Helmet_23_WINT",
    "Virtus_D_2",
    "Tier1_Romeo4T_BCD_G33_Riser_Black_Up",
    "G_Bandanna_Vampire_01",
    "rhsusf_acc_eotech_xps3",
    "Tier1_Romeo4T_BCD_Riser_Tan",
    "Tier1_LeupoldM3A_ADM_Desert",
    "Tier1_MicroT1_Leap_Black",
    "Tier1_URX4_NGAL_M603V_FL",
    "UK3CB_underbarrel_acc_fgrip",
    "kat_crossPanel",
    "mpx_daysack2",
    "mpx_virtus_p",
    "mpx_daysack10",
    "Tier1_Elcan_156_C2_Black",
    "rhsusf_acc_su230a_mrds_c",
    "Tier1_MicroT2_Leap_3xMag_Riser_Black_Up",
    "Tier1_MicroT2_G33_Tan_Up",
    "rhsusf_acc_su230a",
    "Tier1_EXPS3_0_3xMag_Riser_Black_Up",
    "UK3CB_BAF_H_Beret_AAC",
    "5R_Tank_Helmet_23_ESS",
    "rhsusf_cvc_alt_krh_helmet",
    "UK3CB_BAF_AT4_CS_AT_Launcher",
    "SmokeShellBlue",
    "UK3CB_Glasses",
    "ACE_CableTie",
    "Virtus_C_1",
    "UK3CB_BAF_L134A1",
    "5R_Tank_Helmet_26_ESS_WINT",
    "Tier1_MicroT2_Black",
    "Tier1_Elcan_156_C1_Black_2D",
    "UK3CB_underbarrel_acc_grippod",
    "kat_ketamine",
    "tfw_ilbe_DD_gr",
    "Tier1_Romeo4T_BCD_G33_Black_Up",
    "SmokeShell",
    "Chemlight_blue",
    "kat_phenylephrine",
    "RHS_jetpilot_usaf",
    "ACE_salineIV",
    "kat_oxygenTank_150",
    "Tier1_Eotech551_L3_Riser_Black_Up",
    "Tier1_Elcan_156_C1_FDE_2D",
    "Tier1_EXPS3_0_G33_Tano_Up",
    "Tier1_Romeo4T_BCD_Black",
    "Tier1_Harris_Bipod_MVG_MLOK_tan",
    "UK3CB_BAF_H_Beret_RTR_PRR",
    "ACE_splint",
    "ACE_bodyBag_white",
    "mpx_virtus4",
    "ACE_Chemlight_HiBlue",
    "mpx_daysack4",
    "kat_naloxone",
    "SmokeShellPurple",
    "rhsusf_acc_su230a_c",
    "Tier1_MicroT1_Leap_3xMag_Riser_Black_Up",
    "Tier1_Elcan_156_C2_Docter_Black_2D",
    "Tier1_URX4_LA5_M600V_Black",
    "ACE_muzzle_mzls_L",
    "mpx_camel2",
    "5R_Tank_Helmet_27_ESS_ARD_Clean",
    "ACE_rope27",
    "ACE_HandFlare_Red",
    "kat_stethoscope",
    "mpx_virtus_Medic_p",
    "G_Bandanna_tan",
    "Tier1_URX4_LA5_M603V_FL",
    "UK3CB_BAF_U_HeliPilotCoveralls_RN",
    "rhsusf_cvc_green_ess",
]
