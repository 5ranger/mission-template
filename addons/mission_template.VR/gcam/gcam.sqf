...
_CreateGroupMarker =
{
	...
	_ma_mnl = [];
	{
		if (alive _x) then
		{
			_ma_mn = format["gcam_%1%2%3",typeOf _x, rankId _x, name _x];
		...
		{
			deleteMarkerLocal format["gcam_%1",_x];
		};
	} forEach _ma_gu;
};
	...
	while { ( [_sm_cp select 0, _sm_cp select 1] distance [_sm_cp_t_l select 0, _sm_cp_t_l select 1] ) > (_sm_d + (sqrt((velocity _o select 0)^2 + (velocity _o select 1)^2)*0.01)) and (_sm_n < 100) and _sm_l } do
	{
		...
		if (_li) then
		{
			...
			else
			{
				...
				"gcam_o" setMarkerTextLocal "gcam_";
			};
			...
			_ma_mnl = [];
			{
				...
				{
					deleteMarkerLocal format["gcam_%1%2%3",typeOf _x, rankId _x, name _x];
				};
			} forEach _ma_gu;
		};
		...
		if (GCam_MC) then
		{
			...
			if (_ma_sc) then
			{
				...
				(
					(count _ma_ol) > 0
					and
					((findDisplay 5100 displayCtrl 100) posWorldToScreen GCam_MCP) distance ((findDisplay 5100 displayCtrl 100) posWorldToScreen (getMarkerPos format["gcam_%1g",(_ma_ol select 0)])) < 0.02
				)
				then
				{
					lbClear 50;
					...
					deleteMarkerLocal format["gcam_%1g",(_ma_ol select 0)];
					_ma_st = diag_tickTime;
			...
			else
			{
				...
				(
					...
					and
					((findDisplay 5100 displayCtrl 100) posWorldToScreen GCam_MCP) distance ((findDisplay 5100 displayCtrl 100) posWorldToScreen (getMarkerPos format["gcam_%1%2%3",typeOf(driver vehicle(_ma_ol select 0)), rankId(driver vehicle(_ma_ol select 0)), name(driver vehicle(_ma_ol select 0))])) < 0.04
				)
				then
				{
					_ma_sc = true;
