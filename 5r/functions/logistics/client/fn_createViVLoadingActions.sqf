// ViV crate/object loading script. Add object classnames to _crates array to allow them to be loaded into ViV compatible vehicles. 
// Includes check for object variable "C5R_disableViVLoading" which can be used to prevent loading on specific objects: _this setVariable ["C5R_disableViVLoading",true,true];
params ["_crates"];
_actionMain = 
["C5R_loadCrate","Load in closest vehicle (ViV)","",{
...
},{(isNull isVehicleCargo _target) && !(_target getVariable ["C5R_disableViVLoading",false])},{}] call ace_interact_menu_fnc_createAction;
{
